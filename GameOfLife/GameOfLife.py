...
for i in range(testCases):
    ...
    for j in range(generations):
        currentGen = lines[:]
        # We're checking 10 lines of 10 characters each, therefore we're checking 100 characters
        for k in range(100):
            currentLine = k//10  # divide by ten, drop the remainder, get the line number
            # How much between current number and next 10? gets the current index
            currentIndex = k % 10
            alive = False
            surrounds = checkSurroundings(
                currentLine, currentIndex, currentGen)
            ...
            if alive and surrounds == 2 or surrounds == 3:
                ...
                lines[currentLine] = lines[currentLine][:currentIndex] + "1" + \
                    lines[currentLine][currentIndex +
                                       1:]  # Replaces the current character with a 1
                ...
                lines[currentLine] = lines[currentLine][:currentIndex] + "0" + \
                    lines[currentLine][currentIndex +
                                       1:]  # Replaces the current character with a 0
